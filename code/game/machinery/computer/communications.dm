	...
	if (authenticated || issilicon(user))
		...
		switch (ui_state)
			if (STATE_MAIN)
				...
				if (SSshuttle.emergency && SSshuttle.emergency.mode != SHUTTLE_IDLE && SSshuttle.emergency.mode != SHUTTLE_RECALL)
...
/obj/machinery/computer/communications/proc/can_buy_shuttles(mob/user)
	...
	if (!authenticated_as_non_silicon_captain(user))
		return FALSE
	if (SSshuttle.emergency && SSshuttle.emergency.mode != SHUTTLE_RECALL && SSshuttle.emergency.mode != SHUTTLE_IDLE)
